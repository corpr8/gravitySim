...
CANNON.Demo = function(options) {
	...
	var randomSolidMaterial = new THREE.MeshLambertMaterial({
		color : parseInt(getRandomColor(),16)
	});
	//THREE.ColorUtils.adjustHSV(solidMaterial.color, 0, 0, 0.9);
	
	function getRandomMaterial(){
	   var newRandomSolidMaterial = new THREE.MeshLambertMaterial({
		color : parseInt(getRandomColor(),16)
	   });
	   return newRandomSolidMaterial;
	}
	...
	var currentMaterial = randomSolidMaterial;
	...
	function init() {
		...
		scene.fog = new THREE.Fog(0x222222, 10, FAR);
		...
		if (e.keyCode) {
			switch(e.keyCode) {
				...
				case 109:
					...
					idx++;
					idx = idx % renderModes.length;
					// begin at 0 if we exceeded number of modes
	...
	function shape2mesh(shape) {
		...
		switch(shape.type) {
			...
			case CANNON.Shape.types.SPHERE:
				...
				mesh = new THREE.Mesh(sphere_geometry, getRandomMaterial());
				break;
